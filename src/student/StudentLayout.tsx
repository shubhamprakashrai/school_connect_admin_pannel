/**
 * Student portal layout — own sidebar + topbar.
 * Sits under `/student/*` and is gated to users with the STUDENT role.
 */

import { useEffect, useState, type ReactNode } from 'react';
import { NavLink, Outlet, useLocation } from 'react-router-dom';
import {
  GraduationCap, Home, Calendar, User, BookOpen, LogOut, Menu, X,
} from 'lucide-react';
...
const NAV: NavItem[] = [
  { to: '/student',          label: 'Overview',   icon: <Home className="w-5 h-5" />, end: true },
  { to: '/student/attendance', label: 'Attendance', icon: <BookOpen className="w-5 h-5" /> },
  { to: '/student/calendar', label: 'Calendar',   icon: <Calendar className="w-5 h-5" /> },
  { to: '/student/profile',  label: 'My profile', icon: <User className="w-5 h-5" /> },
];
    ...
    <div className="flex h-screen bg-slate-50 dark:bg-slate-950 relative">
      ...
      >
        <div className="h-full bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-r border-slate-100 dark:border-slate-800 flex flex-col">
          <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-br from-cyan-500/10 via-emerald-500/10 to-amber-500/10 pointer-events-none" />
          {/* Brand */}
          <div className="relative flex items-center justify-between gap-2 h-16 px-5 border-b border-slate-100 dark:border-slate-800">
            <div className="flex items-center gap-2">
              <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-cyan-500 via-emerald-500 to-amber-500 text-white flex items-center justify-center shadow-lg font-bold text-sm">
                <GraduationCap className="w-4 h-4" />
              </div>
              <div className="leading-tight">
                <div className="text-[15px] font-bold text-ink-900 dark:text-slate-100 font-display">School Connect</div>
                <div className="text-[10px] text-ink-300 dark:text-slate-500 uppercase tracking-wider">Student portal</div>
              </div>
            </div>
            <button
              onClick={() => setDrawerOpen(false)}
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `group flex items-center px-3 py-2.5 text-sm font-medium rounded-xl transition-all ${
                        isActive
                          ? 'bg-gradient-to-r from-cyan-500 via-emerald-500 to-amber-500 text-white shadow-lg'
        ...
        <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-100 dark:border-slate-800 z-20 sticky top-0">
          <div className="flex items-center justify-between px-6 py-3">
            <button
              ...
              className="p-2 rounded-lg text-ink-500 hover:text-ink-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-100 dark:hover:bg-slate-800 transition focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
            ...
            <div className="flex items-center gap-3">
              <LanguageToggle />
              <ThemeToggle />
              <AccountMenu personaPath="student" onLogout={() => logout()} />
            </div>
          </div>
        </header>
